...
export const useAuthStore = defineStore('auth', () => {
  ...
  const {data, run: fetchSummary, isLoading, isSuccess, isError, error} = useGetUserSummary()
	...
	const webApiKey = ref(query.webApiKey as string || localStorage.getItem(WEB_API_KEY_STORAGE_KEY) || '')
	const username = ref(query.username as string || localStorage.getItem(USERNAME_STORAGE_KEY) || '')
